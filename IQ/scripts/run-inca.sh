          ...
          for ws in "${window_size[@]}"; do
              ...
              for wssf in "${window_size_slide_factor[@]}"; do
                  ...
                  for ((j=1; j<=1; j++)); do
                      ...
                      ip2=$((incons_perc/10))
                      if [ $ip2 = 0 ]; then
                          java -Xmx"$MEMORY" -cp ../target/my-project-name-jar-with-dependencies.jar "com.limos.fr.${name_experiment[$i]}"LinearRoad 1 "$cs" "$((ws*1000))" $(((ws/wssf)*1000)) "$1" "$2" $(($2*2)) "$topic" &> ${name_experiment[$i]}-linearroad.out &
                          wait
                      fi
                      if [ $ip2 != 0 ]; then
                        java -Xmx"$MEMORY" -cp ../target/my-project-name-jar-with-dependencies.jar "com.limos.fr.${name_experiment[$i]}"LinearRoad 1 "$cs" "$((ws*1000))" $(((ws/wssf)*1000)) "$1" "$2" $(($2*2)) "$topic" &> ${name_experiment[$i]}-linearroad.out &
