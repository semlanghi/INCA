...
import com.limos.fr.queries.ProfilingImp;
...
public class QueryImpWithoutJSON {

	private Properties props;

	public QueryImpWithoutJSON(Properties props) {
		this.props = props;
	}

	public String runQuery(JSONObject jo) throws Exception {
		Config.setCon(props);
		String result = runQueryHelp(jo);

		return result;
	}

	private String runQueryHelp(JSONObject jo) throws Exception {
		String query = jo.getString("query");
		QueryEnvironment.Operator operator = jo.getEnum(QueryEnvironment.Operator.class, "operator");
	...
	private String run(String query, int filter, long cstrs, JSONArray measures, QueryEnvironment.Operator operator) throws Exception {
		...
		for(int i = 0; i<measures.length(); i++) {
			String measure = measures.getEnum(QueryEnvironment.Measure.class, i).getDescription();

			List<List<String>> iResult = null;

			if (operator == QueryEnvironment.Operator.TOPK)
...
//	}
	...
	private List<List<String>> runAllAndThreshold(String query, QueryEnvironment.Operator operator, int filter, long cstrs, String measure) throws SQLException {
...
//			val = cbsExpression;
			if (operator != QueryEnvironment.Operator.ALL) {
